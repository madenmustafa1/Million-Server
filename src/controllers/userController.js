...
const myProfileUpdate = async (req, res, next) => {
    ...
    let chatChannel
    if(req.body.hasOwnProperty("chatChannel")){
        chatChannel = req.body.chatChannel
        delete req.body.chatChannel
    }
    ...
    if(error){
        ...
        try{
            ...
            if(chatChannel != null && chatChannel != ""){

                const chatChannelResult = await User.findByIdAndUpdate({_id: req.user.id}, 
                    {$push: {chatChannel: { $each: [ chatChannel ] }}}, {new: true, runValidators: true})
            }
